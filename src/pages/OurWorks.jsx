    ...
    <div>
       ...
       <Navbar2/>
        ...
        <div className='flex flex-col gap-6 justify-center w-full items-center '>
            <h1 className='text-[40px] text-center text-[#5F7487] font-bold leading-10 '>Weaving <br />Dreams into Digital Reality</h1>
            <div className='grid grid-cols-2 md:grid-cols-4'>
                <button className='flex bg-[#F59620] text-white px-[40px] py-4 rounded-md'>Mobile app</button>
                <button className='flex text-black px-[40px] py-4 rounded-md'>Website</button>
                <button className='flex text-black px-[40px] py-4 rounded-md'>UI/UX design</button>
                <button className='flex text-black px-[40px] py-4 rounded-md'>Dashboard</button>
            </div>
            {/* orur project */}
            <div className='grid  sm:grid-cols-2 md:grid-cols-3 gap-5 md:gap-x-10 mt-5 md:gap-y-4 items-center pb-10'>
            {casestudies.map((item)=>(
                <div key={item.id} className='relative flex max-w-[100vh] flex-col gap-[20px]'>
                    <div className='max-h-[100vh]'>
                         <div className=' absolute p-2 rounded-md mt-4  right-2 bg-[#FBECD8] text-[#F59620]'>
                            <span className='font-outfit'>{item.date}</span>
                        </div>
                        
                       <img src={item.image} alt="" width={300} height={0} />
                    </div>
